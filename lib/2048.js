...
// iterates over grid and builds columns
var buildColumn = function(grid, columnIndex) {
  var column = [];
...
var Board = function() {
  ...
  this.thirdRow = createRow();
  this.fourthRow = createRow();

  this.grid = [this.firstRow, this.secondRow, this.thirdRow, this.fourthRow];
  // this.grid = (new Array(4)).map(function (r) { return createRow() });

  this.firstColumn = buildColumn(0);
  this.secondColumn = buildColumn(1);
  this.thirdColumn = buildColumn(2);
  this.fourthColumn = buildColumn(3);
};
